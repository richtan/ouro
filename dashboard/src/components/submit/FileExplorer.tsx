...
function TreeItem({
  ...
  return (
    <div
      className={`group flex items-center gap-1 w-full py-[3px] cursor-pointer transition-colors ${
        isActive
          ? "bg-o-bg text-o-text"
...
export default function FileExplorer({
  ...
  return (
    ...
    >
      ...
      <div className="md:hidden flex items-center gap-1 px-3 py-2 border-b border-o-border overflow-x-auto">
        {files.map((file, i) => (
          <button
            key={i}
            onClick={() => selectFile(i)}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs font-mono whitespace-nowrap flex-shrink-0 transition-colors ${
              i === activeIndex
                ? "bg-o-bg text-o-text"
                : "text-o-textSecondary hover:text-o-text"
            }`}
          >
        ...
        <button
          onClick={addFile}
          className="px-2 py-1.5 text-xs text-o-muted hover:text-o-blueText flex-shrink-0"
        >
          +
        </button>
      </div>
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          {/* Mobile toolbar */}
          <div className="md:hidden flex items-center justify-between px-3 py-2 border-b border-o-border">
            <button
              onClick={() => fileInputRef.current?.click()}
              className="text-xs text-o-textSecondary hover:text-o-blueText transition-colors"
            >
              ↑ Upload
            </button>
