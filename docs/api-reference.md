...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `POST` | `/slurm/v0.0.38/job/submit` | `X-SLURM-USER-TOKEN` header | Submit job via sbatch, wrapped in Docker container. Always requires `workspace_path` + `entrypoint`. Accepts optional `docker_image` (Docker Hub reference), `entrypoint_cmd` (exec-form command), and `dockerfile_content` (for docker build on worker). `image` field is used to resolve Docker Hub reference via `resolve_docker_image()` for non-Dockerfile jobs only; ignored when `dockerfile_content` is present (worker builds custom image). Has a transition fallback for legacy `script` payloads (converts to workspace internally). |
