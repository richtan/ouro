...
3. `submit_to_slurm` — Calls SlurmClient.submit_job() with workspace_path + entrypoint (and optional `docker_image` + `needs_docker_build` + `entrypoint_cmd` from Dockerfile). Workspace is writable (quota-limited to 500 MB / 10K files) and mounted at `/workspace` inside the container. Updates DB status to `running`
