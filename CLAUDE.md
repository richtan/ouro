...
- **Dockerfile builds bypass `resolve_docker_image`** — When `dockerfile_content` is present in the proxy's `wrap_in_docker()`, the `image` field is ignored (a custom image is built via `docker build`). The proxy must skip `resolve_docker_image()` for these jobs, since `image` may contain a raw Docker Hub name (e.g. `ubuntu:22.04`) rather than a prebuilt alias. `resolve_docker_image()` rejects anything not in the `DOCKER_IMAGES` map.
- **MCP changes require a version bump + tag to reach users** — The MCP server is distributed as the `ouro-mcp` npm package. Any change to `mcp/src/` is invisible to users until you: (1) bump the version in all 3 locations (`mcp/package.json`, `mcp/src/index.ts` McpServer constructor, `.mcp/server.json`), (2) commit, (3) tag with `mcp-vX.Y.Z`, and (4) push the tag. CI publishes to npm on tag push. Run `npm view ouro-mcp version` first to ensure the new version is higher. See `mcp/CLAUDE.md` for the full publishing checklist.
