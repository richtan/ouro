...
- **`submit_from_session` advisory lock** — Added `pg_advisory_xact_lock` and `MAX_ACTIVE_JOBS_PER_WALLET` check to match `submit_compute`, preventing concurrent credit/job-count races.
