...
- **Storage + job endpoints require wallet auth** — `GET /api/storage` requires EIP-191 sig. `GET /api/jobs/{job_id}` and `GET /api/jobs/{job_id}/events` require either EIP-191 sig (with submitter wallet match) or admin key. Message formats: `ouro-storage-list:{wallet}:{timestamp}`, `ouro-job-view:{job_id}:{wallet}:{timestamp}`, `ouro-job-events:{job_id}:{wallet}:{timestamp}`. Dashboard proxy routes (`/api/proxy/jobs`, `/api/proxy/credits`) also require wallet sig with message `ouro-list-my-data:{wallet}:{timestamp}`.
